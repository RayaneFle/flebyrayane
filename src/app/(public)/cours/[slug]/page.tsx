...
export default async function CourseDetailPage({ params }: { params: { slug: string } }) {
  ...
  const totalLessons = course.sections.reduce((s, sec) => s + sec.lessons.filter(l => !l.publishAt || new Date(l.publishAt) <= new Date()).length, 0);
            ...
            return (
              <div key={s.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                {s.lessons.length === 0 ? (
                  ...
                  <div>
                    {s.lessons.filter(l => !l.publishAt || new Date(l.publishAt) <= new Date()).map((l, li) => {
