...
export async function POST(request: Request) {
  try {
    ...
    // Si le body contient "code", c'est pour rejoindre une classe
    if (body && typeof body.code !== "undefined") {
      const parsed = JoinSchema.safeParse(body);
      if (!parsed.success) {
        const message = parsed.error.errors[0]?.message || "Code invalide.";
    ...
    // Sinon, c'est pour creer une classe (teacher/admin only)
