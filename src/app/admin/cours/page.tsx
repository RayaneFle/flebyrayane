    ...
    <div>
      ...
      {courses.length === 0 ? <div className="bg-white rounded-2xl border border-brand-100 p-12 text-center"><span className="text-5xl">📖</span><p className="text-slate-400 mt-4">Aucun cours.</p></div> : (
        ...
        <tbody className="divide-y divide-slate-50">{courses.map(c => (
          <tr key={c.id} className="hover:bg-brand-50/30"><td className="px-5 py-4 font-medium text-slate-800">{c.title}</td><td className="px-5 py-4 text-center"><span className={`px-2 py-0.5 rounded text-xs font-bold ${levelColors[c.level]}`}>{c.level}</span></td><td className="px-5 py-4 text-center">{c.enrollmentCode ? <span className="font-mono text-xs bg-brand-100 text-brand-700 px-2 py-0.5 rounded">{c.enrollmentCode}</span> : <span className="text-xs text-slate-300">Libre</span>}</td><td className="px-5 py-4 text-sm text-slate-500">{c.author.name}</td><td className="px-5 py-4 text-center text-sm">{c._count.enrollments}</td>
          <td className="px-5 py-4 text-right space-x-2"><Link href={`/admin/cours/${c.id}`} className="text-xs px-3 py-1.5 bg-brand-50 text-brand-600 rounded-lg hover:bg-brand-100">✏️ Éditer</Link><Link href={`/cours/${c.slug}`} className="text-xs px-3 py-1.5 bg-slate-50 text-slate-600 rounded-lg hover:bg-slate-100">👁 Voir</Link><DuplicateCourseBtn courseId={c.id} /></td></tr>
        ))}</tbody></table></div>
      )}
    </div>
  );
