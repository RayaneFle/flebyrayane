    ...
    <div>
      ...
      {activities.length === 0 ? <div className="bg-white rounded-2xl border border-brand-100 p-12 text-center"><span className="text-5xl">🎮</span><p className="text-slate-400 mt-4">Aucune activité.</p></div> : (
        ...
        <tbody className="divide-y divide-slate-50">{activities.map(a => { const t = activityTypeLabels[a.type] || { emoji:"📝", label:a.type }; const canDelete = isAdmin || a.createdBy.id === session?.user?.id; return (
          <tr key={a.id} className="hover:bg-brand-50/30"><td className="px-5 py-4 font-medium text-slate-800">{a.title}</td><td className="px-5 py-4 text-sm">{t.emoji} {t.label}</td><td className="px-5 py-4 text-center">{a.level ? <span className={`px-2 py-0.5 rounded text-xs font-bold ${levelColors[a.level]}`}>{a.level}</span> : "—"}</td><td className="px-5 py-4 text-center text-sm">{a._count.results}</td>
          <td className="px-5 py-4 text-right flex items-center justify-end gap-2"><Link href={`/activites/${a.id}`} className="text-xs px-3 py-1.5 bg-brand-50 rounded-lg hover:bg-brand-100">Voir</Link><Link href={`/admin/activites/${a.id}/modifier`} className="text-xs px-3 py-1.5 bg-amber-50 text-amber-700 rounded-lg hover:bg-amber-100">Modifier</Link><DuplicateActivityBtn activityId={a.id} />{canDelete && <DeleteBtn id={a.id} />}</td></tr>); })}</tbody></table></div>
      )}
    </div>
  );
}
