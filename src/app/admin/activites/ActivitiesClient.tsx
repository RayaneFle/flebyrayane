...
export default function ActivitiesClient({ myActivities, otherActivities, isAdmin, currentUserId }: { myActivities: Activity[]; otherActivities: Activity[]; isAdmin: boolean; currentUserId: string }) {
  ...
  // Get unique types from current tab (with count)
    ...
    <div>
      ...
      {isAdmin && (
        <div className="inline-flex bg-slate-100 rounded-xl p-1 mb-4">
          <button
            onClick={() => { setTab("mine"); setTypeFilter("all"); }}
          ...
          <button
            onClick={() => { setTab("others"); setTypeFilter("all"); }}
      ...
      {baseList.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-6">
          <button
            ...
            className={"text-xs font-semibold px-3 py-1.5 rounded-full transition-all " + (typeFilter === "all" ? "bg-brand-500 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200")}
          >
          ...
          {typesWithCounts.map(([type, count]) => {
            ...
            return (
              <button
                ...
                className={"inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full transition-all " + (typeFilter === type ? "bg-brand-500 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200")}
              >
                <span>{t.emoji}</span>
              ...
              <div key={a.id} className="bg-white rounded-2xl border border-slate-200 hover:border-brand-300 hover:shadow-md transition-all flex flex-col">
                ...
                <div className="border-t border-slate-100 px-3 py-2 flex items-center justify-between gap-1 bg-slate-50/50 rounded-b-2xl">
                  <div className="flex items-center gap-1">
                    ...
                    <Link href={"/activites/" + a.id} className="text-xs font-semibold px-3 py-1.5 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition-colors" title="Voir">
                      👁
                    </Link>
                  </div>
