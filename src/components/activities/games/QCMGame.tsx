...
export default function QCMGame({ config, onComplete }: { config: any; onComplete: (s: number) => void }) {
  ...
  function pick(i: number) {
    ...
    const n = ok ? correct + 1 : correct;
    setCorrect(n);
    setTimeout(() => {
      if (last) onComplete((n / config.questions.length) * 100);
      else { setIdx(p => p + 1); setSel(null); setFb(false); }
    }, 1500);
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-brand-100 overflow-hidden">
      ...
      <div className="p-6 sm:p-8">
        ...
        <h3 className="font-heading text-xl font-bold text-brand-900 mb-6">{q.question}</h3>
        <div className="space-y-3">
          {q.options.map((opt: any, i: number) => {
            ...
            else cls += "border-brand-50 text-brand-300";
            return (
              <button key={i} onClick={() => pick(i)} disabled={fb} className={cls}>
                <span className="inline-flex items-center gap-3">
                  ...
                  {fb && i === q.correctIndex && <span className="ml-auto">✅</span>}
                  {fb && i === sel && i !== q.correctIndex && <span className="ml-auto">❌</span>}
                </span>
              </button>
            );
          })}
