  ...
  return (
    <div className="tiptap-editor border border-brand-200 rounded-xl overflow-hidden">
      ...
      <div className="flex flex-wrap gap-0.5 p-2 border-b border-brand-100 bg-brand-50/50">
        ...
        <span className="w-px h-6 bg-brand-200 mx-1 self-center" />
        <B active={editor.isActive("bulletList")} onClick={() => editor.chain().focus().toggleBulletList().run()} title="Liste a puces">{"\u2022"} Liste</B>
        <B active={editor.isActive("orderedList")} onClick={() => editor.chain().focus().toggleOrderedList().run()} title="Liste numerotee">1. Liste</B>
        <B active={editor.isActive("blockquote")} onClick={() => editor.chain().focus().toggleBlockquote().run()} title="Citation">{"\u275d"}</B>
        <span className="w-px h-6 bg-brand-200 mx-1 self-center" />
        <B active={editor.isActive({textAlign:"left"})} onClick={() => editor.chain().focus().setTextAlign("left").run()} title="Aligner gauche">{"\u2190"}</B>
        <B active={editor.isActive({textAlign:"center"})} onClick={() => editor.chain().focus().setTextAlign("center").run()} title="Centrer">{"\u2194"}</B>
        <B active={editor.isActive({textAlign:"right"})} onClick={() => editor.chain().focus().setTextAlign("right").run()} title="Aligner droite">{"\u2192"}</B>
        <span className="w-px h-6 bg-brand-200 mx-1 self-center" />
