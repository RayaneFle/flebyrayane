      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <NavLink href="/cours">{t("nav.courses")}</NavLink>
          ...
          <div className="hidden md:flex items-center gap-3"><div className="mr-2 border border-brand-100 rounded-lg p-1"><LangSwitcher /></div>
            {session ? (
                ...
                {profileOpen && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-60 bg-white rounded-2xl shadow-xl border border-brand-100 py-2 z-20 animate-slide-down">
                      ...
                      <Link href="/dashboard" onClick={() => setProfileOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-brand-700 hover:bg-brand-50">📊 {t("nav.dashboard")}</Link>
                      ...
                      <button onClick={() => signOut({ callbackUrl: "/" })} className="flex items-center gap-3 w-full text-left px-4 py-2.5 text-sm text-red-500 hover:bg-red-50">🚪 {t("nav.logout")}</button>
                    </div>
                  </>
                )}
              </div>
              ...
              <>
                <Link href="/login" className="text-sm font-medium text-brand-600 hover:text-brand-700">{t("nav.login")}</Link>
                <Link href="/register" className="text-sm font-semibold bg-gradient-to-r from-brand-500 to-accent-500 text-white px-5 py-2 rounded-xl hover:shadow-glow transition-all">{t("nav.register")}</Link>
              </>
            )}
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-brand-100 py-4 space-y-1 animate-slide-down">
            <MLink href="/" onClick={() => setMenuOpen(false)}>🏠 {t("nav.home")}</MLink>
            <MLink href="/cours" onClick={() => setMenuOpen(false)}>📖 {t("nav.courses")}</MLink>
            <MLink href="/activites" onClick={() => setMenuOpen(false)}>🎮 {t("nav.activities")}</MLink>
            ...
            {session ? (
              <>
                ...
                <button onClick={() => signOut({ callbackUrl: "/" })} className="block w-full text-left px-4 py-2.5 text-red-500 font-medium rounded-xl hover:bg-red-50">🚪 {t("nav.logout")}</button>
              </>
            ) : (
              <>
                <MLink href="/login" onClick={() => setMenuOpen(false)}>🔑 {t("nav.login")}</MLink>
                <MLink href="/register" onClick={() => setMenuOpen(false)}>✨ {t("nav.register")}</MLink>
              </>
            )}
          </div>
        )}
